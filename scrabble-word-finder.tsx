...
import { TWO_LETTER_WORDS } from "./lib/2-letter-words";
import { THREE_LETTER_WORDS } from "./lib/3-letter-words";
import { FOUR_LETTER_WORDS } from "./lib/4-letter-words";
import { FIVE_LETTER_WORDS } from "./lib/5-letter-words";
import { SIX_LETTER_WORDS } from "./lib/6-letter-words";
import { SEVEN_LETTER_WORDS } from "./lib/7-letter-words";
...
export default function Component() {
  ...
  const findWordsWithExtraLetters = (
    word: string,
  ...
  const { possibleWords, wordsWithOneExtra, wordsWithTwoExtra } =
    useMemo(() => {
      if (!letters)
        return {
          possibleWords: [],
      ...
      const withOneExtra: Array<{
        word: string;
        extraLetters: string[];
        value: number;
      }> = [];
      const withTwoExtra: Array<{
        word: string;
        extraLetters: string[];
        value: number;
      }> = [];
        ...
        if (word.length >= 3 && word.length <= 7) {
          ...
          if (canMakeWord(word, letters)) {
            ...
            if (oneExtraLetters.length > 0) {
              withOneExtra.push({
                word,
                extraLetters: oneExtraLetters,
                value: wordValue,
              });
            } else if (twoExtraLetters.length > 0) {
              withTwoExtra.push({
                word,
                extraLetters: twoExtraLetters,
                value: wordValue,
              });
            }
          }
      ...
      return {
        possibleWords: possible.sort(
          (a, b) => b.value - a.value || a.word.localeCompare(b.word),
        ),
        wordsWithOneExtra: withOneExtra.sort(
          (a, b) => b.value - a.value || a.word.localeCompare(b.word),
        ),
        wordsWithTwoExtra: withTwoExtra.sort(
          (a, b) => b.value - a.value || a.word.localeCompare(b.word),
        ),
      };
    }, [letters]);
  ...
  const groupedWords = useMemo(() => {
    ...
    Object.keys(groups).forEach((key) => {
      groups[Number.parseInt(key)].sort(
        (a, b) => b.value - a.value || a.word.localeCompare(b.word),
      );
    });
    return groups;
  ...
  const oneExtraTotalPages = Math.ceil(
    wordsWithOneExtra.length / ITEMS_PER_PAGE,
  );
  const twoExtraTotalPages = Math.ceil(
    wordsWithTwoExtra.length / ITEMS_PER_PAGE,
  );
                              ...
                              <div className="grid grid-cols-1 lg:grid-cols-2 gap-2">
                                {words.map(({ word, value }) => (
                                  ...
                                  >
                                    ...
                                    <span className="text-xs bg-green-700 px-2 py-1 rounded ml-2">
                                      {value}
                                    </span>
                                  </Badge>
                                ))}
                ...
                <Card className="border border-blue-800 bg-gray-900">
                  <CardHeader>
                    ...
                    <p className="text-sm text-blue-400">
                      These words can be made if you find the missing letter on
                      the board
                    </p>
                  </CardHeader>
                  <CardContent>
                    {wordsWithOneExtra.length > 0 ? (
                      <div className="space-y-3">
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                          {paginatedOneExtra.map(
                            ({ word, extraLetters, value }) => (
                              <div
                                key={word}
                      ...
                      <p className="text-blue-400 italic">
                        No additional words found with one extra letter.
                      </p>
                    )}
                  </CardContent>
                </Card>
                ...
                <Card className="border border-purple-800 bg-gray-900">
                  <CardHeader>
                    ...
                    <p className="text-sm text-purple-400">
                      These words can be made if you find two missing letters on
                      the board
                    </p>
                  </CardHeader>
                  <CardContent>
                    {wordsWithTwoExtra.length > 0 ? (
                          ...
                          {paginatedTwoExtra.map(
                            ({ word, extraLetters, value }) => (
                              ...
                              >
                                ...
                                <div className="flex gap-1">
                                  <span className="text-sm text-purple-400 mr-2">
                                    Need:
                                  </span>
                      ...
                      <p className="text-purple-400 italic">
                        No additional words found with two extra letters.
                      </p>
                    )}
                  </CardContent>
                </Card>
